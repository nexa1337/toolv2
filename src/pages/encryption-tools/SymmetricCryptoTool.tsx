import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { ArrowRight, Copy, Check, Key, ArrowLeft } from 'lucide-react';
import CryptoJS from 'crypto-js';

type SymmetricAlgorithm = 'AES' | 'DES';

interface SymmetricCryptoToolProps {
  algorithm: SymmetricAlgorithm;
  title: string;
  description: string;
  mode: 'encrypt' | 'decrypt';
}

export function SymmetricCryptoTool({ algorithm, title, description, mode }: SymmetricCryptoToolProps) {
  const [input, setInput] = useState('');
  const [secretKey, setSecretKey] = useState('');
  const [output, setOutput] = useState('');
  const [copied, setCopied] = useState(false);
  const [error, setError] = useState('');

  const handleProcess = () => {
    setError('');
    if (!secretKey) {
      setError('A secret key is required.');
      return;
    }

    try {
      let result = '';
      if (mode === 'encrypt') {
        if (algorithm === 'AES') {
          result = CryptoJS.AES.encrypt(input, secretKey).toString();
        } else if (algorithm === 'DES') {
          result = CryptoJS.DES.encrypt(input, secretKey).toString();
        }
      } else {
        if (algorithm === 'AES') {
          const bytes = CryptoJS.AES.decrypt(input, secretKey);
          result = bytes.toString(CryptoJS.enc.Utf8);
        } else if (algorithm === 'DES') {
          const bytes = CryptoJS.DES.decrypt(input, secretKey);
          result = bytes.toString(CryptoJS.enc.Utf8);
        }
        
        if (!result) {
          throw new Error('Decryption failed. Invalid key or corrupted data.');
        }
      }
      setOutput(result);
    } catch (err) {
      setError(`Failed to ${mode} data. Please check your input and secret key.`);
      setOutput('');
    }
  };

  const handleCopy = () => {
    if (output) {
      navigator.clipboard.writeText(output);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    }
  };

  return (
    <div className="container mx-auto max-w-4xl px-4 py-8 md:px-6 lg:px-8">
      <Link to="/encryption-tools" className="mb-6 inline-flex items-center gap-2 text-sm font-medium text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-50">
        <ArrowLeft className="h-4 w-4" />
        Back to Encryption Tools
      </Link>
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">{title}</h1>
        <p className="mt-2 text-zinc-500 dark:text-zinc-400">
          {description}
        </p>
      </div>

      <div className="space-y-6">
        <div className="space-y-2 max-w-md">
          <label className="text-sm font-medium text-zinc-900 dark:text-zinc-50">Secret Key</label>
          <div className="relative">
            <Key className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-500 dark:text-zinc-400" />
            <input
              type="text"
              value={secretKey}
              onChange={(e) => setSecretKey(e.target.value)}
              placeholder="Enter your secret key..."
              className="h-10 w-full rounded-lg border border-zinc-200 bg-white pl-10 pr-4 text-sm focus:border-zinc-900 focus:outline-none focus:ring-1 focus:ring-zinc-900 dark:border-zinc-800 dark:bg-zinc-950 dark:focus:border-zinc-50 dark:focus:ring-zinc-50"
            />
          </div>
        </div>

        <div className="grid gap-6 md:grid-cols-2">
          <div className="space-y-2">
            <label className="text-sm font-medium text-zinc-900 dark:text-zinc-50">Input {mode === 'encrypt' ? 'Text' : 'Ciphertext'}</label>
            <textarea
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder={`Enter text to ${mode}...`}
              className="min-h-[200px] w-full resize-none rounded-xl border border-zinc-200 bg-white p-4 text-sm focus:border-zinc-900 focus:outline-none focus:ring-1 focus:ring-zinc-900 dark:border-zinc-800 dark:bg-zinc-950 dark:focus:border-zinc-50 dark:focus:ring-zinc-50"
            />
          </div>

          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-zinc-900 dark:text-zinc-50">Output {mode === 'encrypt' ? 'Ciphertext' : 'Text'}</label>
              <button
                onClick={handleCopy}
                disabled={!output}
                className="flex items-center gap-2 text-xs font-medium text-zinc-500 hover:text-zinc-900 disabled:opacity-50 dark:text-zinc-400 dark:hover:text-zinc-50"
              >
                {copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                {copied ? 'Copied!' : 'Copy'}
              </button>
            </div>
            <textarea
              value={output}
              readOnly
              placeholder="Output will appear here..."
              className="min-h-[200px] w-full resize-none rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-sm dark:border-zinc-800 dark:bg-zinc-900/50"
            />
            {error && <p className="text-sm text-red-500">{error}</p>}
          </div>
        </div>

        <button
          onClick={handleProcess}
          disabled={!input || !secretKey}
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-zinc-900 px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-zinc-800 disabled:opacity-50 dark:bg-zinc-50 dark:text-zinc-900 dark:hover:bg-zinc-200 md:w-auto"
        >
          {mode === 'encrypt' ? `Encrypt with ${algorithm}` : `Decrypt ${algorithm}`}
          <ArrowRight className="h-4 w-4" />
        </button>
      </div>
    </div>
  );
}
