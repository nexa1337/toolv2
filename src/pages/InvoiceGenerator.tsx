...
const t = {
  en: {
    invoice: "Invoice", quote: "Invoice", customerName: "Customer Name", billingAddress: "Billing Address", shippingAddress: "Shipping Address",
    currency: "Currency", invoiceNum: "Invoice #", referenceNum: "Reference #", date: "Date", dueDate: "Expiration Date",
    ...
    customerInfo: "Customer Information", terms: "Terms & Conditions", attachFiles: "Attach file(s) (Max 3 files, 10MB each)",
  ...
  fr: {
    invoice: "Facture", quote: "Devis", customerName: "Nom du client", billingAddress: "Adresse de facturation", shippingAddress: "Adresse de livraison",
    currency: "Devise", invoiceNum: "Devis #", referenceNum: "N° de référence", date: "Date du Devis", dueDate: "Date d’expiration",
    ...
    customerInfo: "Informations sur le client", terms: "Conditions générales", attachFiles: "Joindre le(s) fichier(s) à Devis (Max 3 fichiers, 10 Mo chacun)",
...
interface InvoiceItem { desc: string; qty: number; price: number; tax: number; }
interface Attachment { name: string; size: number; dataUrl: string; }
interface Contact { name: string; email: string; }
interface InvoiceData {
  lang: "en" | "fr"; currency: string; invoiceNum: string; referenceNum: string; date: string; dueDate: string;
...
const defaultData: InvoiceData = {
  lang: "fr", currency: "EUR", invoiceNum: "DEV-001", referenceNum: "REF-123", date: new Date().toISOString().split("T")[0], dueDate: new Date(Date.now() + 30*24*60*60*1000).toISOString().split("T")[0],
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.bold}>{lang.invoiceNum}: {data.invoiceNum}</Text>
...
export function InvoiceGenerator() {
  const [data, setData] = useState<InvoiceData>(() => storage.get("invoice_data_v3", defaultData));
  const debouncedData = useDebounce(data, 500);
  const [instance, updateInstance] = usePDF({ document: <InvoicePDF data={debouncedData} /> });
  ...
  useEffect(() => {
    storage.set("invoice_data_v3", data);
  }, [data]);

  useEffect(() => {
    updateInstance(<InvoicePDF data={debouncedData} />);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    files.forEach(file => {
      ...
      reader.onload = (ev) => {
        setData(prev => ({
          ...prev,
          attachments: [...prev.attachments, { name: file.name, size: file.size, dataUrl: ev.target?.result as string }]
        }));
      };
      reader.readAsDataURL(file);
    });
  ...
  const handleDownloadPNG = async () => {
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        if (blob) saveAs(blob, `${data.invoiceNum}.png`);
      });
    } catch (err) {
  ...
  const handleDownloadWord = async () => {
    ...
    children.push(
      ...
      new Paragraph({ text: `${lang.invoiceNum}: ${data.invoiceNum}` }),
    ...
    Packer.toBlob(doc).then(blob => {
      saveAs(blob, `${data.invoiceNum}.docx`);
    });
  };
      ...
      <div className={cn("w-full overflow-y-auto border-r border-zinc-200/60 p-4 sm:p-6 lg:w-[55%] dark:border-zinc-800/60", activeTab === "preview" ? "hidden lg:block" : "block")}>
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-zinc-500 dark:text-zinc-400 mt-1">Create professional quotes instantly.</p>
          </div>
          <div className="flex items-center gap-2">
            <ProjectManager
              storageKey="invoice_projects_v1"
              currentData={data}
              onLoad={(newData) => setData(newData)}
              titleExtractor={(d) => d.invoiceNum}
              typeLabel="Invoice"
            />
            <select 
        ...
        <div className="space-y-6 pb-20">
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              <div className="space-y-2"><Label>{lang.invoiceNum}</Label><Input value={data.invoiceNum} onChange={e => updateField("invoiceNum", e.target.value)} /></div>
          ...
          <Card>
            <CardHeader><CardTitle>{lang.attachFiles}</CardTitle></CardHeader>
            <CardContent>
              <label className="border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl p-8 flex flex-col items-center justify-center text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors cursor-pointer">
                <Upload className="h-8 w-8 mb-2" />
                <p className="text-sm font-medium">Click to upload or drag and drop</p>
                <p className="text-xs mt-1">Max 3 files, 10MB each</p>
                ...
                <div className="mt-4 space-y-2">
                  {data.attachments.map((file, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-zinc-200 dark:border-zinc-800">
                      ...
                      <Button variant="ghost" size="icon" className="text-red-500 hover:bg-red-100 dark:hover:bg-red-900/30" onClick={() => removeAttachment(i)}>
        ...
        <div className="flex flex-wrap gap-2 mb-4 justify-end">
          ...
          <Button size="sm" onClick={() => {
            if (instance.url) {
              ...
              link.download = `${data.invoiceNum}.pdf`;
              link.click();
            }
          }}>
